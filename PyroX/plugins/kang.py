...
from Barath.helpers.help_func import get_args
from Barath import barath
from config import HANDLER, OWNER_ID 



@barath.on_message(filters.command("kang", prefixes=HANDLER) & filters.user(OWNER_ID))
async def kang(client, message):
    ...
    if replied and replied.media:
        ...
        photo = await barath.download_media(message=replied)
    else:
    ...
    if photo:
        ...
        packname = f"a{user.id}_by_teamx_{pack}"
        ...
        if exist is not False:
            try:
                await barath.send_message("Stickers", "/addsticker")
            except YouBlockedUser:
                await message.edit("first **unblock** @Stickers")
                return
            await barath.send_message("Stickers", packname)
            ...
            while limit in await get_response(message):
                ...
                if await get_response(message) == "Invalid pack selected":
                    await barath.send_message("Stickers", cmd)
                    await get_response(message)
                    await barath.send_message("Stickers", packnick)
                    await get_response(message)
                    await barath.send_document("Stickers", photo)
                    await get_response(message)
                    await barath.send_message("Stickers", emoji_)
                    await get_response(message)
                    await barath.send_message("Stickers", "/publish")
                    if is_anim:
                        await get_response(message)
                    ...
                    await get_response(message)
                    await barath.send_message("Stickers", "/skip")
                    await get_response(message)
                    await barath.send_message("Stickers", packname)
            ...
            await barath.send_message("Stickers", emoji_)
            await get_response(message)
            await barath.send_message("Stickers", "/done")
        else:
            ...
            if is_anim:
                await get_response(message)
                await barath.send_message("Stickers", f"<{packnick}>", parse_mode=None)
            await get_response(message)
            await barath.send_message("Stickers", "/skip")
            await get_response(message)
            await barath.send_message("Stickers", packname)
        ...
        await barath.read_history("Stickers")
        if os.path.exists(str(photo)):
            os.remove(photo)


@barath.on_message(filters.command("sinfo", prefixes=HANDLER) & filters.user(OWNER_ID))
async def sticker_pack_info_(client, message):
    ...
    get_stickerset = await barath.send(
        GetStickerSet(
...
async def get_response(message):
    return [x async for x in barath.iter_history("Stickers", limit=1)][0].text
